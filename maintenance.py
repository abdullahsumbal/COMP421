...
class Maintenance:
    def __init__(self, db, table_name='booking'):
        ...
        is_area_clean = ['Yes', 'No']
        are_plants_watered= ['Yes', 'No']
        additional_comments = 'None'
        cursor.execute("select serial_no  from spinbike;")
        serial_no = cursor.fetchall()
        serial_no_len = len(serial_no)
        ...
        for i in range(num_of_records):
            ...
            cursor.execute("""INSERT INTO maintenance (maintenance_date, bmemail, place_id, are_plants_watered, is_area_clean, does_computer_work, additional_comments, serial_no) VALUES (%(maintenance_date)s, %(bmemail)s, %(place_id)s, %(are_plants_watered)s, %(is_area_clean)s, %(does_computer_work)s, %(additional_comments)s, %(serial_no)s);""" ,
                           {'maintenance_date': maintenance_date,
                            'bmemail': bmemail,
                            'place_id': place_id,
                            'are_plants_watered': are_plants_watered[random.randrange(2)],
                            'is_area_clean': is_area_clean[random.randrange(2)],
                            'does_computer_work': does_computer_work[random.randrange(2)],
                            'additional_comments': additional_comments,
                            'serial_no': serial_no[random.randrange(serial_no_len)][0]})
            db.commit()
